...
`agent-workspace-linux` server uses OMP's native MCP support; no extension is required. Restart OMP, run
`/mcp test agent-workspace-linux`, then invoke its manual-only workflow with
`/skill:agent-workspace-linux <task>`.
